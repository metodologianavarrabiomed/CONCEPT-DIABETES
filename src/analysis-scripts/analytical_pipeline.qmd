...
df <- bind_rows(df_else,df_ob,df_f,df_erc,df_ic,df_ecv)
...
df_random_effect_haz <- 
  df_base_haz %>% 
  mutate(hazard_center1 = hazard_base * exp(ranef(me_model)$cluster[1]),
     hazard_center2 = hazard_base * exp(ranef(me_model)$cluster[2]),
     hazard_center3 = hazard_base * exp(ranef(me_model)$cluster[3]),
     hazard_center4 = hazard_base * exp(ranef(me_model)$cluster[4]))
...
df_survival <- 
  ...
  mutate(cum_hazard_base = map_dbl(index, calc_chf, hazard_colname = "hazard_base"), 
         ...
         cum_hazard_center3 = map_dbl(index, calc_chf, hazard_colname = "hazard_center3"), 
         cum_hazard_center4 = map_dbl(index, calc_chf, hazard_colname = "hazard_center4"), 
         
         surv_base = exp(-cum_hazard_base), 
         surv_center1 = exp(-cum_hazard_center1),
         surv_center2 = exp(-cum_hazard_center2),
         surv_center3 = exp(-cum_hazard_center3),
         surv_center4 = exp(-cum_hazard_center4)
  )
  

# plot the derived survival curves: ----
df_survival %>% 
  pivot_longer(cols = c(surv_base, surv_center1, surv_center2, surv_center3, surv_center4), 
               names_to = "survival") %>% 
  ggplot(aes(x = time, 
             y = value, 
             group = survival, 
