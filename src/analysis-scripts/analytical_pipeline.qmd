...
fig1.savefig("fitness_by_year.png")
...
DATEDIFF('day', initial_date,final_date) AS t_1, fitness, initial_date, status \
...
fig2.savefig("fitness2cond_by_year.png")
...
df <- dbGetQuery(con, paste0("SELECT period2process_fitness.ID, period2process_fitness.t_0, period2process_fitness.t_1, period2process_fitness.fitness, period2process_fitness.status, patient_incidents_view.age, patient_incidents_view.sex, patient_incidents_view.copayment FROM period2process_fitness LEFT JOIN patient_incidents_view ON period2process_fitness.ID=patient_incidents_view.patient_id"))
dbDisconnect(con, shutdown = TRUE)
...
png(filename=gsub("src/analysis-scripts/","",
    ...
    width = 600, height = 750, units = "px")
plot(mj3, which = "fit", var.time = "t_1", marg = F, break.times = 5, bty = "l", legend.loc = 'bottomright',
     ylab = "fitness", xlab = "Time in days",shades=TRUE,ylim = c(0.1,0.9))
dev.off()
 
d1 <- data.frame(t_1=seq(1,1200,length.out=100))
data.new <-data.frame(d1,age=0,sex=0)
mj3.pred <- predictY(mj3, data.new , var.time = "t_1")
...
```
![Class-specific weighted marginal and subject-specific mean predicted trajectories](../../outputs/joint1_PI.png){#fig-joint1_PI fig-align="center" width="80%"}

![Class-specific fitness mean  predicted trajectory](../../outputs/joint2_PI.png){#fig-joint2_PI fig-align="center" width="80%"}
