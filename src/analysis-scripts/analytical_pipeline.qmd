            ...
            DATE_ADD(month_nac, INTERVAL 65 YEAR) AS 'turn_to_65' \
                FROM main.patient WHERE patient_id IN (SELECT patient_id \
                        FROM main.patient WHERE dx_date >='2017-01-01' \
                         AND (death_date IS NULL OR death_date>dx_date))")
            ...
            SELECT * FROM comorb_incidents_view_all_following \
                LEFT JOIN cie2ciap \
                    ON comorb_incidents_view_all_following.comorb_codif = cie2ciap.comorb_codif \
                        AND comorb_incidents_view_all_following.comorb_code = cie2ciap.CIE")
...
#| tbl-cap: "Demographic and socioeconomic characteristics of incident patients "
#| tbl-colwidths: [60,40]

###INCIDENTS
patient_inc <- dbGetQuery(con,
                       "SELECT p.*,c.initial_status AS PCC,
                        CASE WHEN p.sex = 0 THEN 'Male' 
                        ELSE 'Female' END AS sexo 
                        FROM patient_incidents_view_ch p 
                        LEFT JOIN patient_condition_translation c 
                        ON p.patient_id=c.patient_id") 
...
```{r}
#| label: patient_inc_by_condition_0

followup_years <- dbGetQuery(con,
        "SELECT PCC, SUM(followup_years) AS total_followup_years FROM \
        (SELECT pc.patient_id,pc.initial_status AS PCC, \
        DATEDIFF('day',piv.dx_date ,pc.date)/365.25 AS followup_years\
        FROM patient_condition_hosp pc LEFT JOIN patient_incidents_view piv \
         ON pc.patient_id=piv.patient_id) GROUP BY PCC")  
n <- followup_years$PCC
followup_years <- as.data.frame(t(followup_years[,-1])) %>%
    mutate(Total = rowSums(select_if(., is.numeric)))
colnames(followup_years) <- append(n,"Total")
followup_years <- followup_years[c("else", "ob", "f",  "ckd",  "hf", "cvd", "Total")]

hba1c_by_pcc <- dbGetQuery(con,
        "SELECT PCC, SUM(n) AS total_measurements FROM \
        (SELECT COUNT(*) AS n, patient_id,ANY_VALUE(initial_status) AS PCC\
        FROM (SELECT * FROM param_incidents_view param LEFT JOIN (SELECT \ 
        pc.patient_id,pc.initial_status,pc.date as followup_end, piv.dx_date \
        FROM patient_condition_hosp pc LEFT JOIN patient_incidents_view piv \
        ON pc.patient_id=piv.patient_id) followup \
        ON param.patient_id=followup.patient_id \
        WHERE param.param_name='hba1c' \
        AND param.param_date>=followup.dx_date \
        AND param.param_date<followup.followup_end) GROUP BY patient_id) GROUP BY PCC")
n <- hba1c_by_pcc$PCC
hba1c_by_pcc <- as.data.frame(t(hba1c_by_pcc[,-1])) %>%
    mutate(Total = rowSums(select_if(., is.numeric)))
colnames(hba1c_by_pcc) <- append(n,"Total")
hba1c_by_pcc <- hba1c_by_pcc[c("else", "ob", "f",  "ckd",  "hf", "cvd", "Total")]

atc_by_pcc <- dbGetQuery(con,
        "SELECT PCC, atc_code, SUM(treatment_days)/365.25 AS treatment_years\ 
        FROM (SELECT DISTINCT t.patient_id, t.atc_code,t.prescription_date_ini,\
        DATEDIFF('day',t.prescription_date_ini,\
        LEAST(COALESCE(t.prescription_date_end,CAST('2022-12-31' AS DATE)),followup.followup_end)) as treatment_days, \
        followup.initial_status AS PCC \
        FROM treat_incidents_view t LEFT JOIN (SELECT \ 
        pc.patient_id,pc.initial_status,pc.date as followup_end, piv.dx_date \
        FROM patient_condition_hosp pc LEFT JOIN patient_incidents_view piv \
        ON pc.patient_id=piv.patient_id) followup ON t.patient_id=followup.patient_id \
        WHERE t.dispensing_date>=followup.dx_date \
        AND t.dispensing_date<followup.followup_end) GROUP BY PCC, atc_code") 
code2drug <- read.csv("./diabetes_drugs.csv")
n <- code2drug$X
code2drug <- as.data.frame(t(code2drug[,-1]))
colnames(code2drug) <- n
code2drug$atc_code <- factor(row.names(code2drug))

repeat_rows <- function(df) {
  # Create an empty list to store the repeated rows
  repeated_rows <- list()
  
  # Loop through each row in the original dataframe
  for (i in 1:nrow(df)) {
    # Split the values by '+' symbol
    split_values <- strsplit(df$class[i], "\\+")[[1]]
    
    # For each part, repeat the row and assign the respective part to 'values'
    for (j in seq_along(split_values)) {
      new_row <- df[i, , drop = FALSE]  # Copy the original row
      new_row$class <- split_values[j]  # Assign the current part to 'values'
      repeated_rows[[length(repeated_rows) + 1]] <- new_row  # Append the new row
    }
  }
  
  # Combine all the repeated rows back into a new dataframe
  result_df <- do.call(rbind, repeated_rows)
  return(result_df)
}
tryCatch({
  code2drug <- repeat_rows(code2drug)
  atc_by_pcc <- atc_by_pcc %>%
      left_join(code2drug[c('atc_code','class')], by = "atc_code") %>%
      drop_na() %>%
      group_by(PCC, class) %>%
      summarise(treatment_years = sum(treatment_years, na.rm = TRUE), .groups = "drop") %>%
      complete(PCC,class,fill=list(treatment_years=0)) %>%
      pivot_wider(
          names_from = PCC,
          values_from = treatment_years,
      ) %>%
      mutate(Total = rowSums(select_if(., is.numeric)))
  row_names <- atc_by_pcc$class
  atc_by_pcc <- atc_by_pcc[c("else", "ob", "f",  "ckd",  "hf", "cvd", "Total")]
  
  hba1c_per_year <- hba1c_by_pcc/followup_years
  hba1c_per_year_upper_ci <- hba1c_per_year+1.96*sqrt(
    hba1c_per_year/followup_years
  )
  hba1c_per_year_lower_ci <- hba1c_per_year-1.96*sqrt(
    hba1c_per_year/followup_years
  )
  treat_per_year <- mapply('/', atc_by_pcc, followup_years[1,])
  treat_per_year_upper_ci <- treat_per_year+1.96*sqrt(
    mapply('/',(1-treat_per_year)*treat_per_year,followup_years[1,]))
  treat_per_year_lower_ci <- treat_per_year-1.96*sqrt(
    mapply('/',(1-treat_per_year)*treat_per_year,followup_years[1,]))
  
  patient_inc <- patient_inc %>% mutate(sex=sexo)
  dependent <- 'PCC' 
  explanatory=c("sex","age_dx","CC","copayment","education", "avg_income","charlson")
  patient_inc$PCC <- factor(patient_inc$PCC,levels=c("else", "ob", "f",  "ckd",  "hf", "cvd"))},
error = function(e) {
            message("error in patient_inc_by_condition_0")
        }
)
...
```{r}
#| label: tbl-patient_inc_by_condition_1
#| tbl-cap: "Demographic and socioeconomic characteristics of incident patients by different predominant clinical conditions"
#| tbl-colwidths: [60,40]
...
combine_matrices <- function(mat1, mat2, mat3, mat4) {
  ...
  for (i in 1:nrow) {
    for (j in 1:ncol) {
      # Combine the values as required
      result[i, j] <- paste(
        mat1[i, j], " (", max(mat2[i, j],0.00), ",", mat3[i, j], "); ", mat4[i, j], sep = "")
    }
  }
...
tryCatch({
  hba1c_per_year <- combine_matrices(
    round(hba1c_per_year,2),round(hba1c_per_year_lower_ci,2),round(hba1c_per_year_upper_ci,2),
    matrix(hba1c_by_pcc,nrow=nrow(hba1c_by_pcc),ncol=ncol(hba1c_by_pcc)))
  treat_per_year <- combine_matrices(
    round(100*treat_per_year,1),round(100*treat_per_year_lower_ci,1),round(100*treat_per_year_upper_ci,1),
    round(atc_by_pcc,2))
  
  per_year <- as.data.frame(cbind(
      matrix(c("Followup total years","HbA1c patient-year rate", paste("Percentage of time covered by ",row_names, " prescription"),
               "Number of years", "Rate (95% CI); n",rep("% (95% CI); years",length(row_names))),
      nrow = length(row_names)+2, ncol = 2),
      rbind(
        matrix(round(followup_years,2),nrow=nrow(followup_years),ncol=ncol(followup_years)),
        hba1c_per_year,treat_per_year))
  )
  colnames(per_year) <- colnames(patient_inc_tab)
  
  patient_inc_tab <- rbind(patient_inc_tab, per_year)
  kable(patient_inc_tab, row.names=FALSE, align=c("l", "l", rep("r",5)))},
error = function(e) {
            message("error in patient_inc_by_condition_1")
        }
)
```
...
#| tbl-cap: "Demographic and socioeconomic characteristics of prevalent patients"
#| tbl-colwidths: [60,40]
...
![Percentage of patient traces an activity is present during the first year](../../outputs/activity_presence_process_indicators_year1.png){#fig-activity_presence_year1 fig-align="center" width="80%"}

![Percentage of patient traces an activity is present during the second year](../../outputs/activity_presence_process_indicators_year2.png){#fig-activity_presence_year2 fig-align="center" width="80%"}

![Percentage of patient traces an activity is present during the third year](../../outputs/activity_presence_process_indicators_year3.png){#fig-activity_presence_year3 fig-align="center" width="80%"}

![Percentage of patient traces an activity is present during the fourth year](../../outputs/activity_presence_process_indicators_year4.png){#fig-activity_presence_year4 fig-align="center" width="80%"}

![Percentage of patient traces an activity is present during the fifth year](../../outputs/activity_presence_process_indicators_year5.png){#fig-activity_presence_year5 fig-align="center" width="80%"}
...
#| warning: FALSE
#| output: TRUE
log_info("Create KM plot for PI")
con <- dbConnect(duckdb(), dbdir = db_path)
...
survfit(Surv(y, status) ~ 1, data=df %>% mutate(y=t_1/365.25) %>% 
             group_by(ID) %>%
             slice_max(order_by = t_1, n = 1)) %>%
     autoplot(xlab = "Years", ylab = "Survival probability")

ggsave(filename=gsub("src/analysis-scripts/","",
                      here("outputs","KM_PI.png")),
        plot=last_plot(),width = 800, height = 700, units = "px",dpi=150)
```
...
![Percentage of patient traces an activity is present](../../outputs/activity_presence_else.png){#fig-activity_presence_else fig-align="center" width="80%"}
...
con <- dbConnect(duckdb(), dbdir = db_path)
survfit(Surv(y, status) ~ 1, data=dbGetQuery(con, paste0("SELECT *, t_1/365.25 as y FROM (SELECT piv.*,p2f.*,
                ROW_NUMBER() OVER (PARTITION BY p2f.ID ORDER BY p2f.t_1 DESC) AS rn
         FROM period2fitness_",cond," p2f \
                              LEFT JOIN patient_incidents_view_ch piv \
                              ON p2f.ID=piv.patient_id) WHERE rn=1"))) %>%
    autoplot(xlab = "Years", ylab = "Survival probability")
ggsave(filename=gsub("src/analysis-scripts/","",
                     here("outputs",sprintf("KM_%s.png",
                                            cond))),
       plot=last_plot(),width = 800, height = 700, units = "px",dpi=150)
dbDisconnect(con, shutdown = TRUE)
```
...
![Percentage of patient traces an activity is present](../../outputs/activity_presence_ob.png){#fig-activity_presence_ob fig-align="center" width="80%"}
...
![Percentage of patient traces an activity is present](../../outputs/activity_presence_f.png){#fig-activity_presence_f fig-align="center" width="80%"}
...
![Percentage of patient traces an activity is present](../../outputs/activity_presence_ckd.png){#fig-activity_presence_ckd fig-align="center" width="80%"}
...
![Percentage of patient traces an activity is present](../../outputs/activity_presence_hf.png){#fig-activity_presence_hf fig-align="center" width="80%"}
...
![Percentage of patient traces an activity is present](../../outputs/activity_presence_cvd.png){#fig-activity_presence_cvd fig-align="center" width="80%"}
