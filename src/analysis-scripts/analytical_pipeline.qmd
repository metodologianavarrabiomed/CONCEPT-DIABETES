...
def select_by_condition(cond):
    ...
    con = duckdb.connect('C:/Users/D463112/Desktop/ejemplo_cdmb/inputs/data.duckdb')
...
def small_cluster_filter(log,min_traces=25,col_id='ID',col_clust='cluster'):
...
The function below takes an event log and returns each patients' period's fitness.
...
def id2fitness_by_interval(xes_file,pn_file,ini_place='place100',fin_place='place111',
  ...
  '''
    ...
    period2fitness.to_csv(output_csv,index=False)
...
The link between guideline adherence, in terms of performed process measures, and clinical outcomes is a highly demanded issue in diabetes care. Joint models are used to analyse simultaneously two related phenomena, the evolution of a variable and the occurence of an event. Joint latent class models (JLCM) consist of a linear mixed model and a proportional hazard model linked by the latent classes. The population is split in several groups, the latent classes, and each class is caracterized by a specific evolution of the dependent variable and an associated risk of event. Using fitness as time dependent treatment adherence measure we can made a joint latent class model.  
### Cox Regression Model with Mixed Effects

The Cox proportional hazards regression model is frequently used for the analysis of survival data. The inclusion of random effects into a Cox proportional hazards model shares many similarities with methods for the analysis for multilevel data with continuous, binary or count outcomes. A conventional regression model (in this case the Cox proportional hazards model) is enhanced through the incorporation of random effect terms to account for within-cluster homogeneity in outcomes.
## Results
...
In the next example, for simplicity, we choose fitness, age, sex and copayment to try to predict any predominant clinical condition change, and the summary of it is:
...
id2fitness_by_interval(xes_file='../../outputs/eventlog_filtered_%s.xes' % cond,
            pn_file='./PN_%s.pnml' % cond,
            ini_place='place100',fin_place='place111',
            fixed_period_time=90,
            output_csv='../../outputs/fitness_by_period_%s.csv' % cond)
```
...
df_fitness <- read.csv(gsub("src/analysis-scripts/","",
                            here("outputs",sprintf("fitness_by_period_%s.csv",
                                                   cond))))
#df_fitness <- read.csv("C:/Users/D463112/Desktop/pruebas/CONCEPT-DIABETES/outputs/fitness_by_period_else.csv")
#df_patient <- read.csv("C:/Users/D463112/Desktop/pruebas/CONCEPT-DIABETES/outputs/dm_patients_df_else.csv")
df_patient <- read.csv(gsub("src/analysis-scripts/","",
                            here("outputs",sprintf("dm_patients_df_%s.csv",
                                                   cond))))
df <- merge(x=df_patient[c('patient_id','age','sex','copayment','fin_cause')],
            y=df_fitness[c('ID','t_0','t_1','fitness','status')],  
            by.x=c("patient_id"), 
            by.y=c("ID"),
            all.x=FALSE)
...
### 1 latent class
df <- df %>% 
  mutate(statusCR=case_when(fin_cause=="['death']" ~ 2,
                                       fin_cause=="" ~ 0,
                                       TRUE ~ 1)) %>% 
  filter(statusCR!=2)
...
mj1 <- Jointlcmm(fixed= fitness ~ t_1 ,random =~ t_1, subject="ID2",
                 survival = Surv(t_min,t_max,statusCR) ~ age+sex, hazard="Weibull",
...
mj3 <- gridsearch(rep = 10, maxiter = 15, minit = mj1, 
                  Jointlcmm(fixed= fitness ~ t_1 ,mixture =~ t_1,
                            ...
                            survival = Surv(t_min,t_max,statusCR)~ age+sex,
...
plot(model, which = "fit", var.time = "t_1", marg = F, break.times = 10, bty = "l", 
     ylab = "fitness", xlab = "Time in days",shades=TRUE)

par(mfrow=c(1,2))
plot(model, which = "fit", var.time = "t_1", marg = F, break.times = 10, bty = "l", 
     ylab = "fitness", xlab = "Time in days",subset = statusCR == 0,
     main="no complication",shades=TRUE)
plot(model, which = "fit", var.time = "t_1", marg = F, break.times = 10, bty = "l", 
     ylab = "fitness", xlab = "Time in days",subset = statusCR == 1,
     main="predominal condition change",shades=TRUE)
d1 <- data.frame(t_1=seq(1,1000,length.out=100))
data.new <-data.frame(d1,age=0,sex=0)
model.pred <- predictY(model, data.new , var.time = "t_1")


plot(model.pred, bty = "l", ylim = c(0, 1), legend.loc = FALSE,
     ylab = "fitness", xlab = "days", lwd = 2,col=c("red","navy","green"))

plot(model, which = "survival", lwd = 2, legend.loc = FALSE, bty = "l",
     xlab = "days", ylab = "event-free probability",col=c("red","navy","green"))
...
#| warning: FALSE
#| output: TRUE
df_cluster_else <- read.csv(gsub("src/analysis-scripts/","",
                            here("outputs","fitness_by_cluster_else.csv")))
df_patient_else <- read.csv(gsub("src/analysis-scripts/","",
                            here("outputs","dm_patients_df_else.csv")))
df_else <- merge(x=df_patient_else[c('patient_id','age','sex','copayment',
                                    'dx_date','fin_date','fin_cause')],
                 y=df_cluster_else[c('ID','cluster')],  
                 by.x=c("patient_id"), 
                 by.y=c("ID"),
                 all.x=FALSE)
df_else$cluster <- paste(df_else$cluster,"_else",sep="")

df_cluster_ob <- read.csv(gsub("src/analysis-scripts/","",
                            here("outputs","fitness_by_cluster_ob.csv")))
df_patient_ob <- read.csv(gsub("src/analysis-scripts/","",
                            here("outputs","dm_patients_df_ob.csv")))
df_ob <- merge(x=df_patient_ob[c('patient_id','age','sex','copayment',
                                   'dx_date','fin_date','fin_cause')],
                 y=df_cluster_ob[c('ID','cluster')],  
                 by.x=c("patient_id"), 
                 by.y=c("ID"),
                 all.x=FALSE)
df_ob$fin_cause <- ifelse(df_ob$fin_cause == "['bmi<30']", "", df_ob$fin_cause)
df_ob$cluster <- paste(df_ob$cluster,"_ob",sep="")

df_cluster_f <- read.csv(gsub("src/analysis-scripts/","",
                            here("outputs","fitness_by_cluster_f.csv")))
df_patient_f <- read.csv(gsub("src/analysis-scripts/","",
                            here("outputs","dm_patients_df_f.csv")))
df_f <- merge(x=df_patient_f[c('patient_id','age','sex','copayment',
                                   'dx_date','fin_date','fin_cause')],
                 y=df_cluster_f[c('ID','cluster')],  
                 by.x=c("patient_id"), 
                 by.y=c("ID"),
                 all.x=FALSE)
df_f$cluster <- paste(df_f$cluster,"_f",sep="")

df_cluster_erc <- read.csv(gsub("src/analysis-scripts/","",
                            here("outputs","fitness_by_cluster_erc.csv")))
df_patient_erc <- read.csv(gsub("src/analysis-scripts/","",
                            here("outputs","dm_patients_df_erc.csv")))
df_erc <- merge(x=df_patient_erc[c('patient_id','age','sex','copayment',
                                   'dx_date','fin_date','fin_cause')],
                 y=df_cluster_erc[c('ID','cluster')],  
                 by.x=c("patient_id"), 
                 by.y=c("ID"),
                 all.x=FALSE)
df_erc$cluster <- paste(df_erc$cluster,"_erc",sep="")

df_cluster_ic <- read.csv(gsub("src/analysis-scripts/","",
                            here("outputs","fitness_by_cluster_ic.csv")))
df_patient_ic <- read.csv(gsub("src/analysis-scripts/","",
                            here("outputs","dm_patients_df_ic.csv")))
df_ic <- merge(x=df_patient_ic[c('patient_id','age','sex','copayment',
                                   'dx_date','fin_date','fin_cause')],
                 y=df_cluster_ic[c('ID','cluster')],  
                 by.x=c("patient_id"), 
                 by.y=c("ID"),
                 all.x=FALSE)
df_ic$cluster <- paste(df_ic$cluster,"_ic",sep="")

df_cluster_ecv <- read.csv(gsub("src/analysis-scripts/","",
                            here("outputs","fitness_by_cluster_ecv.csv")))
df_patient_ecv <- read.csv(gsub("src/analysis-scripts/","",
                            here("outputs","dm_patients_df_ecv.csv")))
df_ecv <- merge(x=df_patient_ecv[c('patient_id','age','sex','copayment',
                                   'dx_date','fin_date','fin_cause')],
                 y=df_cluster_ecv[c('ID','cluster')],  
                 by.x=c("patient_id"), 
                 by.y=c("ID"),
                 all.x=FALSE)
df_ecv$cluster <- paste(df_ecv$cluster,"_ecv",sep="")

df <- bind_rows(df_else,df_ob,df_f,df_erc,df_ic,df_ecv) %>% filter(fin_cause!="['death']")

df$sex <- as.factor(df$sex)
df$cluster <- as.factor(df$cluster)
df$status <- ifelse(is.na(df$fin_cause) | df$fin_cause != "", 1, 0)
df$fin_date [df$fin_date ==""] <- "2023-01-01"
df <- df %>%
  mutate(n_days = as.Date(fin_date) - as.Date(dx_date))

df$n_days <- as.numeric(df$n_days)



base_model <- coxph(Surv(n_days, status) ~ age + sex, data=df)
#summary(base_model)
me_model <- coxme(Surv(n_days, status) ~ age + sex + (1 | cluster), data=df)
summary(me_model)
#marginal.model <- coxph(Surv(n_days, status) ~ age + sex, data=df, cluster=cluster)
#summary(marginal.model)
#frailty.model  <- coxph(Surv(n_days, status) ~ age + sex + frailty(cluster), data=df)
#summary(frailty.model)
predicted <- predict(me_model, type = "risk")

# Compute the c-index
library(glmnet)
c_index <- Cindex(predicted, y = me_model$y)
```



```{r}
#| label: cluster survival model
#| eval: TRUE
#| echo: FALSE
#| warning: FALSE
#| output: TRUE
#https://cran.r-project.org/web/packages/coxme/vignettes/coxme.pdf
nchange <- with(df, tapply(status,cluster,sum,na.rm=T))
pyears <- with(df, tapply(n_days, cluster, sum, na.rm=T))/365.25
count <- with(df, tapply(status, cluster,function(x) sum(!is.na(x))))
indx <- match(names(ranef(me_model)[[1]]), names(nchange)) 
 
plot(nchange[indx], exp(ranef(me_model)[[1]]), log='y',
 xlab="Number of changes per cluster",
 ylab="Estimated cluster risk")
abline(h=1, lty=2)
dev.off()

indx <- match(c(72,165), names(nchange))
temp <- cbind(nchange, count, pyears, 100*nchange/pyears)[indx,]
 dimnames(temp) <- list(indx,
 c("Changes", "N", "Years of FU", "Rate"))
print(round(temp,2))
...
loglik <- double(15)
for (i in 1:15) {
 tfit <- coxme(Surv(n_days, status) ~ age + sex + (1 | cluster),
               data=df,vfixed=estvar[i])
 loglik[i] <- 2*diff(tfit$loglik)[1]
 }
plot(sqrt(estvar), loglik,
 xlab="Std of the random effect", ylab="2 * loglik")
abline(h=2*diff(me_model$loglik)[1] - qchisq(.95, 1), lty=2)
dev.off()
                
temp <- 2*diff(me_model$loglik)[1] - loglik
approx(temp[1:8], sqrt(estvar[1:8]), 3.84)$y

## Get profile likelihood for variance values to examine, based on
## observed .486
s <- seq(0.01, 0.4, length.out = 200)^2
res <- sapply(s, function(i) {
    mod <- coxme(Surv(n_days, status) ~ age + sex + 
        (1 | cluster), data = df, vfixed = i)
    2 * diff(mod$loglik)[1]
})
...
abline(h = 2 * diff(me_model$loglik)[1] - qchisq(0.95, 1), lty = 2)
...
names_list <- as.list(names(me_model$frail$cluster))

for (i in c(1:length(names_list))) {
  df_base_haz <- df_base_haz %>% 
    mutate(!!paste("hazard_center",names_list[[i]],sep="_") 
           := df_base_haz[['hazard_base']] * exp(ranef(me_model)$cluster[i]))
}
df_random_effect_haz <- df_base_haz
df_survival <- 
  df_random_effect_haz  %>%  
  mutate(cum_hazard_base = map_dbl(index, calc_chf,
                                   hazard_colname = "hazard_base"))

for (i in c(1:length(names_list))) {
  df_survival <- df_survival %>%
    mutate(!!paste("cum_hazard_center",names_list[[i]],sep="_")
           := map_dbl(index, calc_chf, hazard_colname = paste("hazard_center",
                                                              names_list[[i]],
                                                              sep="_")))
}
df_survival <- 
  df_survival  %>%  
  mutate(surv_base = exp(-cum_hazard_base))

for (i in c(1:length(names_list))) {
  df_survival <- df_survival %>% 
    mutate(!!names_list[[i]] := exp(-df_survival[[4+length(names_list)+i]]))
}


# plot the derived survival curves: ----
df_survival %>% 
  pivot_longer(cols = c((5+2*length(names_list)):(5+3*length(names_list))), 
               names_to = "survival") %>% 
  ggplot(aes(x = time, 
             y = value, 
             group = survival, 
             col = survival)) +
  geom_point() + 
  scale_x_continuous(limits = c(0, max(df_survival$time))) + 
