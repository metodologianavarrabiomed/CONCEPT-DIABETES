...
```{r}
#| label: tbl-patient_inc_by_condition_0
#| tbl-cap: "Demographic and socioeconomic characteristics of incident patients by different predominant clinical conditions"
#| tbl-colwidths: [60,40]

hba1c_per_personyear_by_pcc <- dbGetQuery(con,
          "SELECT patient_condition_hosp.patient_id AS patient_id,\
          coalesce(n,0) AS hba1c_per_personyear FROM patient_condition_hosp \
          LEFT JOIN (SELECT COUNT(*)/ANY_VALUE(followup_days)*365.25 AS n,patient_id \
          FROM (SELECT *,DATEDIFF('day',dx_date, followup_end) as followup_days \
          FROM param_incidents_view param LEFT JOIN (SELECT \ 
          ...
          AND param.param_date<followup.followup_end) GROUP BY patient_id) t \
          ON patient_condition_hosp.patient_id=t.patient_id")

atc_per_personyear <- dbGetQuery(con,
                "SELECT patient_id, atc_code, \
                COUNT(*)/ANY_VALUE(followup_days)*365.25 AS prescriptions_per_year,ANY_VALUE(PCC) AS PCC \
                FROM (SELECT DISTINCT t.patient_id, t.atc_code,\
                DATEDIFF('day',followup.dx_date, followup.followup_end) as followup_days, \
                prescription_date_ini, followup.initial_status AS PCC \
                FROM treat_incidents_view t LEFT JOIN (SELECT \ 
          pc.patient_id,pc.initial_status,pc.date as followup_end, piv.dx_date \
          FROM patient_condition_hosp pc LEFT JOIN patient_incidents_view piv \
          ON pc.patient_id=piv.patient_id) followup ON t.patient_id=followup.patient_id WHERE t.dispensing_date>=followup.dx_date \
          AND t.dispensing_date<followup.followup_end) GROUP BY patient_id, atc_code") 
code2drug <- read.csv("C:/Users/D463112/Desktop/pruebas/CONCEPT-DIABETES/src/analysis-scripts/diabetes_drugs.csv")
n <- code2drug$X
code2drug <- as.data.frame(t(code2drug[,-1]))
colnames(code2drug) <- n
code2drug$atc_code <- factor(row.names(code2drug))
atc_per_personyear <- atc_per_personyear %>%
    left_join( code2drug[c('atc_code','class')], by = "atc_code") %>%
    drop_na() %>%
    group_by(patient_id, class) %>%
    summarise(total_value = sum(prescriptions_per_year, na.rm = TRUE), .groups = "drop") %>%
    complete(patient_id, class, fill = list(total_value = 0)) %>%
    pivot_wider(
        names_from = class,
        values_from = total_value,
    )
names(atc_per_personyear) <- gsub("\\+", "and", names(atc_per_personyear))
treatments <- names(atc_per_personyear[,2:dim(atc_per_personyear)[2]])
patient_inc <- patient_inc %>%
  left_join( hba1c_per_personyear_by_pcc, by = "patient_id") %>%
  left_join(atc_per_personyear, by = "patient_id")
patient_inc[, treatments][is.na(patient_inc[,treatments])] <- as.numeric(0.0)

explanatory=append(c("age_dx","CC","copayment","education", "avg_income","charlson","hba1c_per_personyear"),treatments)
dependent <- 'PCC' 
...
patient_inc%>%summary_factorlist(dependent,
                              ...
                              include_col_totals_percent	=FALSE,
                              add_col_totals = TRUE)-> patient_inc_tab
patient_inc_tab$label <- gsub("and", "+", patient_inc_tab$label)
kable(patient_inc_tab, row.names=FALSE, align=c("l", "l", rep("r",5)))
```
