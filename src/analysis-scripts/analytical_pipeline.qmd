...
def evlog_creation_prescriptions(con,
    ...
    param_df = con.sql(f"SELECT patient_id,param_date AS date, \
                       CASE WHEN param_value < {7.5} THEN 'HBA<75' \
                           ELSE 'HBA>75' END AS Event,\
                       ROW_NUMBER() OVER (ORDER BY param_date) + {act_n} AS actins\
                           FROM param_incidents_view \
                               WHERE patient_id IN (SELECT patient_id \
                                FROM patient_condition \
                                    WHERE patient_condition.type = '{cond}') \
    ...
    df = con.sql(f"SELECT *, 'start' AS cycle,\
                 ROW_NUMBER() OVER (ORDER BY patient_id, date) AS actins \
                     FROM (SELECT patient_id,param_date AS date, \
                       CASE WHEN param_value < {7.5} THEN 'HBA<75' \
                           ELSE 'HBA>75' END AS Event, 0 AS nac_code\
                           FROM param_incidents_view \
                               WHERE patient_id IN (SELECT patient_id \
                                FROM patient_condition \
                                    WHERE patient_condition.type = '{cond}') \
                     ...
                     FROM (SELECT patient_id,param_date AS date, param_date,\
                       CASE WHEN param_value < {7.5} THEN 'HBA<75' \
                           ...
                           FROM param_incidents_view \
                            ...
                            CAST(date AS TIMESTAMP)+INTERVAL {treat_min_days} DAYS AS date, \
                                date AS param_date, \
...
Depending on the results of the clustering we may need to focus our analysis in the biggest clusters and forget those clusters that contains too less traces. The function to clust takes into account that to clusterize the patients by their traces:

```{python}
def clust(con,cond,clust_n,dist_matrix,df_,id2trace,patients):
...
def id2fitness_by_interval(con,
                           cond,
                           pn_file,
                           ini_place='place100',
                           fin_place='place111',
...
#| echo: FALSE
#| output: TRUE
logging.info("Make k-elbow figure of %s patients", cond)
if len(traces)>25:
...
con = duckdb.connect(db_path)
clust(con,cond,n_clusters,dm,df_,id2trace,patients)
del dm
del df_
```
...
#| output: FALSE
con = duckdb.connect(db_path)
id2fitness_by_interval(con, cond,
            pn_file='./PN_%s.pnml' % cond,
...
#df_fitness <- con.sql(f"SELECT * FROM period2fitness_{cond}")
...
#            by.y=c("ID"),
#            all.x=FALSE)
df <- dbExecute(con, paste0("SELECT period2fitness_",cond,".ID,period2fitness_",cond,".t_0,period2fitness_",cond,".t_1,period2fitness_",cond,".fitness,period2fitness_",cond,".status,patient_incidents_view.age,patient_incidents_view.sex,patient_incidents_view.copayment,patient_incidents_view.fin_cause FROM period2fitness_",cond," LEFT JOIN patient_incidents_view ON period2fitness_",cond,".ID=patient_incidents_view.patient_id"))
