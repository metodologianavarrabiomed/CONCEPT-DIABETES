...
combine_matrices <- function(mat1, mat2, mat3) {
  ...
  for (i in 1:nrow) {
    for (j in 1:ncol) {
      ...
      result[i, j] <- paste(
        mat1[i, j], " (", max(mat2[i, j],0.00), ",", mat3[i, j], ")", sep = "")
    }
  }
...
per_year <- as.data.frame(cbind(
    matrix(c("HbA1c patient-year rate", paste("Percentage of time covered by ",row_names, " prescription"),
             "Rate (95% CI)",rep("% (95% CI)",length(row_names))),
    nrow = length(row_names)+1, ncol = 2),
    rbind(hba1c_per_year,treat_per_year))
)
