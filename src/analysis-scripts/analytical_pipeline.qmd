            ...
            SELECT patient_id, admission_date, diagnosis1_code FROM (SELECT *,\
                      ROW_NUMBER() OVER(PARTITION BY patient_id \
                          ...
                          FROM cmbd_incidents_postdx_view WHERE \
                              diagnosis1_code IN ('E10.10', 'E10.11', 'E10.641', 'E11.00', 'E11.01', 'E11.10', 'E11.11', 'E11.641', 'J41.0', 'J41.1', 'J41.8', 'J42', 'J43.0', 'J43.1', 'J43.2', 'J43.8', 'J43.9', 'J44.0', 'J44.1', 'J44.9', 'J47.0', 'J47.1', 'J47.9', 'I09.81', 'I11.0', 'I13.0', 'I13.2', 'I50.1', 'I50.20', 'I50.21', 'I50.22', 'I50.23', 'I50.30', 'I50.31', 'I50.32', 'I50.33', 'I50.40', 'I50.41', 'I50.42', 'I50.43', 'I50.810', 'I50.811', 'I50.812', 'I50.813', 'I50.814', 'I50.82', 'I50.83', 'I50.84', 'I50.89', 'I50.9', 'E87.0', 'E87.1', 'E86.0', 'E86.1', 'E86.9', 'I20.0', 'I20.1', 'I20.8', 'I20.9', 'I24.0', 'I24.8', 'J45.20', 'J45.21', 'J45.22', 'J45.31', 'J45.32', 'J45.41', 'J45.42', 'J45.51', 'J45.52', 'J45.901', 'J45.902', 'J45.990', 'J45.991', 'J45.998', 'J96.0')) t WHERE rn = 1;")
...
Healthcare event log is made in this section with the services visited by incident patients.
...
comorb_ch <- comorb(archivo = dbGetQuery(con,
        "SELECT patient_id AS id,comorb_code AS cod,\
...
dependent="sexo"
explanatory=c("age_dx","CC","copayment","education", "avg_income")  
...
![Predominant clinical condition's first change in incident patients. The number of the boxes shows the number of people experienced the change and the colour of the box the number of days needed to change. ](../../outputs/predominant_condition_matrix.png){#fig-predominant_cond_m fig-align="center" width="80%"}
...
#### Treatments' event log creation and description
...
#### Healthcare trayectories description
...
![Visits' dotted chart](../../outputs/healthcare_process_matrix_else.png){#fig-healthcare_process_matrix_else fig-align="center" width="80%"}
...
In the next example, we choose fitness, age, sex and copayment to try to predict a complication in diabetes (understood a complication as death, a potentially avoidable hospitalization or a predominal clinical condition change to 'cvd','hf' or 'ckd') with a time dependet Cox model, and the summary of it is:
...
survfit(Surv(t_min, t_max, status2) ~ class, data=df %>%
  ...
  autoplot(xlab = "Day", ylab = "Survival probability")
ggsave(filename=gsub("src/analysis-scripts/","",
...
##### Two time windows Cox Model

Moreover a Cox survival model with two time windows it is implemented below:

```{r}
#| label: 2 window cox (else)
#| eval: TRUE
#| echo: FALSE
#| warning: FALSE
#| output: TRUE
...
![Visits' dotted chart](../../outputs/healthcare_process_matrix_ob.png){#fig-healthcare_process_matrix_ob fig-align="center" width="80%"}
...
![Visits' dotted chart](../../outputs/healthcare_process_matrix_f.png){#fig-healthcare_process_matrix_f fig-align="center" width="80%"}
...
![Visits' dotted chart](../../outputs/healthcare_process_matrix_ckd.png){#fig-healthcare_process_matrix_ckd fig-align="center" width="80%"}
...
![Visits' dotted chart](../../outputs/healthcare_process_matrix_hf.png){#fig-healthcare_process_matrix_hf fig-align="center" width="80%"}
...
![Visits' dotted chart](../../outputs/healthcare_process_matrix_cvd.png){#fig-healthcare_process_matrix_cvd fig-align="center" width="80%"}
...
df2finess = con.sql("SELECT d.*,\
              ...
              CASE WHEN age_<50 THEN '[0,50)'\
              WHEN age_>=50 AND age_<60 THEN '[50,60)' \
              WHEN age_>=60 AND age_<70 THEN '[60,70)' \
              WHEN age_>=70 AND age_<80 THEN '[70,80)' \
              WHEN age_>=80 AND age_<90 THEN '[80,90)' \
              ELSE '[90,+)' END AS age\
...
Now treatments' analysis' and process indicators' fitness are linked with the next plot:
```{python}
...
In the next example, we choose fitness, age, sex and copayment to try to predict a complication in diabetes (understood a complication as death, a potentially avoidable hospitalization or a predominal clinical condition change to 'cvd','hf' or 'ckd') with a time dependent Cox model, and the summary of it is:
...
df <- dbGetQuery(con, paste0("SELECT p2p.ID, p2p.t_0, p2p.t_1, p2p.fitness, p2p.status,p2p.status2, piv.age_dx as age, piv.sex, piv.copayment,piv.charlson FROM period2process_fitness p2p LEFT JOIN patient_incidents_view_ch piv ON p2p.ID=piv.patient_id"))
dbDisconnect(con, shutdown = TRUE)
...
A Cox survival model with two time windows it is implemented below:
...
#| tbl-cap: "Models summary"
#| tbl-colwidths: [60,40]
df_cond <- function(cond,m1,m2,m3){
...
results <- rbind(
  df_cond('No predominant condition',cox2w_else,time_dep_else,mj2else),
  df_cond('Obesity',cox2w_ob,time_dep_ob,mj2ob),
  df_cond('Fragility',cox2w_f,time_dep_f,mj2f),
  df_cond('Chronic kidney disease',cox2w_ckd,time_dep_ckd,mj2ckd),
  df_cond('Heart failure',cox2w_hf,time_dep_hf,mj2hf),
  df_cond('Cardiovascular disease',cox2w_cvd,time_dep_cvd,mj2cvd),
  df_cond('Process indicators',cox2w_PI,time_dep_PI,mj2PI)
) %>%
    mutate(
...
kable(results, row.names=FALSE, align=c("l", "l", rep("r",5)))
```
