...
def select_by_condition(con):
    ...
    con.sql("CREATE VIEW param_incidents_view AS \
            SELECT patient_id,param_name,MAX(param_value) AS param_value,param_date\
    ...
    erc_list = set(con.sql(f"SELECT DISTINCT patient_id \
                           FROM comorb_incidents_active_view WHERE CIAP2 = '{erc}'"
                           ).df()['patient_id'])        
    ic_list = set(con.sql(f"SELECT DISTINCT patient_id \
                          FROM comorb_incidents_active_view WHERE CIAP2 = '{ic}'"
                          ).df()['patient_id'])        
    ecv_list = set(con.sql(f"SELECT DISTINCT patient_id \
                           FROM comorb_incidents_active_view \
                               WHERE CIAP2 IN {to_list(ecv_code_list)}"
                               ).df()['patient_id'])        
    ...
    from_ecv_change_list = ['death']
    from_ic_change_list = from_ecv_change_list+to_ecv_change_list
            ...
            SELECT patient_id, param_name, param_date, param_value \
                FROM (SELECT patient_id, param_name, param_date, param_value,\
                      ROW_NUMBER() OVER(PARTITION BY patient_id, param_name \
                          ...
                          FROM param_incidents_predx_view) t WHERE rn = 1;")    
    ...
    erc_list.update(set(con.sql(f"SELECT DISTINCT patient_id \
                           FROM cmbd_incidents_long_view sub \
                               WHERE code IN {to_list(to_erc_change_list)} \
                                   AND admission_date <= (SELECT dx_date \
                        FROM patient_incidents_view \
                            WHERE patient_incidents_view.patient_id = sub.patient_id);"
                            ).df()['patient_id']))        
    ic_list.update(set(con.sql(f"SELECT DISTINCT patient_id \
                           FROM cmbd_incidents_long_view sub \
                               WHERE code IN {to_list(to_ic_change_list)} \
                                   AND admission_date <= (SELECT dx_date \
                        FROM patient_incidents_view \
                            WHERE patient_incidents_view.patient_id = sub.patient_id);"
                            ).df()['patient_id']))        
    ecv_list.update(set(con.sql(f"SELECT DISTINCT patient_id \
                           FROM cmbd_incidents_long_view sub \
                               WHERE code IN {to_list(to_ecv_change_list)} \
                                   AND admission_date <= (SELECT dx_date \
                        FROM patient_incidents_view \
                            WHERE patient_incidents_view.patient_id = sub.patient_id);"
                            ).df()['patient_id']))        
    ...
    con.sql(eval('f"CREATE TABLE patient_condition AS {}"'.format(' UNION ALL '.join(
        [query_function(cond) for cond in ['ecv','ic','erc','f','ob','else']]))))    
    return presc_data_p
                

def evlog_creation_prescriptions(con,
                                 cond,
...
def evlog_creation_dispensations(con, 
                              cond,
                              code2drug_path='./diabetes_drugs.csv',
...
### Data preprocessing and process indicators' event log creation

With the objective of studying process indicators a function to make a process indicator's event log is code above. There, in addition to laboratory measures, specialist visits, and patient interviews, three artificial events have been included to aid the analysis. The 'INI' event denotes each patient's diabetes diagnosis date, the 'FIN' event indicates the date 3 years after the diagnosis, and the 'yFIN' events indicate the end of each annual interval.

```{python, warning: FALSE}
#| label: Preprocessing functions and process indicators' event log
#| warning: FALSE
def evlog_process_ind(con):
    ...
    con.sql(f"CREATE VIEW pre_process_ind AS \
            SELECT *,'start' AS cycle,\
                ROW_NUMBER() OVER (ORDER BY patient_id,date) AS actins \
            ...
            UNION ALL SELECT DISTINCT patient_id,'trgl' AS event, \
                    CAST(param_date AS TIMESTAMP) + INTERVAL 180 SECOND AS date \
            FROM param_incidents_view WHERE param_name='trgl' \
            UNION ALL SELECT DISTINCT patient_id,'creat' AS event, \
                    CAST(param_date AS TIMESTAMP) + INTERVAL 240 SECOND AS date \
            FROM param_incidents_view WHERE param_name='creat' \
            UNION ALL SELECT DISTINCT patient_id,'alb' AS event, \
                    CAST(param_date AS TIMESTAMP) + INTERVAL 300 SECOND AS date \
            FROM param_incidents_view WHERE param_name='alb' \
            UNION ALL SELECT DISTINCT patient_id,'ophtalmologist' AS event, \
                    CAST(visit_date AS TIMESTAMP) + INTERVAL 360 SECOND AS date \
            FROM ss_use_incidents_view WHERE visit_service='OFT' \
            ...
            FROM patient_incidents_view UNION \
            ALL SELECT patient_id, 'FIN' AS event,\
                    CAST(dx_date AS TIMESTAMP) + INTERVAL 1095 DAYS + INTERVAL 1 SECOND AS date \
            FROM patient_incidents_view)  ") 
    
    
    
    con.sql(f"CREATE TABLE process_ind AS SELECT a.* FROM eventlog_guidelines a \
            JOIN (SELECT patient_id, MIN(date) as ini_date,MAX(date) as fin_date \
            FROM eventlog_guidelines WHERE event IN ('INI', 'FIN') \
            GROUP BY patient_id) b ON a.patient_id = b.patient_id \
            WHERE (a.date BETWEEN b.ini_date AND b.fin_date) AND \
                b.fin_date<= (SELECT COALESCE(deregistration_date,'2023-01-01')\
                              FROM  patient_incidents_view \
                                  WHERE a.patient_id = patient_incidents_view.patient_id)")
...
```{python}
#| label: Fitness  calculation by intervals
  ...
  '''
    ...
    for id in tqdm(id_list):
        ...
        for n in range(1,n_periods+2):
            ...
            if n==n_periods+1 and hosp_d==date_max:
                status.append(1)
            else:
                status.append(0)
    ...
    con.sql(f"CREATE TABLE period2fitness_{cond} AS SELECT * FROM period2fitness")
```
...
Process indicators can also be represented in a Petri Net. Therefore we created the next Petri Nets to analyze whether the traces obey the guidelines the first three years:
...
def id2process_fitness(con,
    ...
    Args:
      ...
      date_n_col (str): events' dates' column's name in event log
    Returns:
...
notnan_p = select_by_condition(con)
con.close()
```
...
df <- dbGetQuery(con, paste0("SELECT period2fitness_",cond,".ID,period2fitness_",cond,".t_0,period2fitness_",cond,".t_1,period2fitness_",cond,".fitness,period2fitness_",cond,".status,patient_incidents_view.age,patient_incidents_view.sex,patient_incidents_view.copayment FROM period2fitness_",cond," LEFT JOIN patient_incidents_view ON period2fitness_",cond,".ID=patient_incidents_view.patient_id"))
dbDisconnect(con, shutdown = TRUE)
...
mj1 <- Jointlcmm(fixed= fitness ~ t_1 ,random =~ t_1, subject="ID2",
                 survival = Surv(t_min,t_max,status) ~ age+sex, hazard="Weibull",
                     hazardtype="PH",ng=1,data=df, verbose=FALSE)
### 3 latent class
tryCatch(
{mj3 <- gridsearch(rep = 10, maxiter = 15, minit = mj1, 
                  Jointlcmm(fixed= fitness ~ t_1 ,mixture =~ t_1,
                            random=~t_1,subject="ID2",
                            survival = Surv(t_min,t_max,status)~ age+sex,
                            hazard="Weibull",hazardtype="PH",
                            ng=3,data=df, verbose=FALSE))
...
png(filename=gsub("src/analysis-scripts/","",
                  here("outputs",sprintf("joint1_%s.png",
                                         cond))),
    width = 600, height = 750, units = "px")
plot(mj3, which = "fit", var.time = "t_1", marg = F, break.times = 10, bty = "l", legend.loc = 'bottomright',
     ylab = "fitness", xlab = "Time in days",shades=TRUE,ylim = c(0.1,0.9))
dev.off()
 
d1 <- data.frame(t_1=seq(1,1000,length.out=100))
data.new <-data.frame(d1,age=0,sex=0)
mj3.pred <- predictY(mj3, data.new , var.time = "t_1")
...
```
![Class-specific weighted marginal and subject-specific mean predicted trajectories](../../outputs/joint1_else.png){#fig-joint1_else fig-align="center" width="80%"}

![Class-specific fitness mean  predicted trajectory](../../outputs/joint2_else.png){#fig-joint2_else fig-align="center" width="80%"}
...
```{python}
#| label: create fitness by period table (ecv)
#| eval: TRUE
#| echo: FALSE
#| warning: FALSE
#| output: FALSE
...
```
![Class-specific weighted marginal and subject-specific mean predicted trajectories](../../outputs/joint1_ecv.png){#fig-joint1_ecv fig-align="center" width="80%"}

![Class-specific fitness mean  predicted trajectory](../../outputs/joint2_ecv.png){#fig-joint2_ecv fig-align="center" width="80%"}

![Class-specific survival functions](../../outputs/joint3_ecv.png){#fig-joint3_ecv fig-align="center" width="80%"}
...
```{python}
#| label: create fitness by period table (ic)
#| eval: TRUE
#| echo: FALSE
#| warning: FALSE
#| output: FALSE
...
```
![Class-specific weighted marginal and subject-specific mean predicted trajectories](../../outputs/joint1_ic.png){#fig-joint1_ic fig-align="center" width="80%"}

![Class-specific fitness mean  predicted trajectory](../../outputs/joint2_ic.png){#fig-joint2_ic fig-align="center" width="80%"}

![Class-specific survival functions](../../outputs/joint3_ic.png){#fig-joint3_ic fig-align="center" width="80%"}
...
```{python}
#| label: create fitness by period table (erc)
#| eval: TRUE
#| echo: FALSE
#| warning: FALSE
#| output: FALSE
...
```
![Class-specific weighted marginal and subject-specific mean predicted trajectories](../../outputs/joint1_erc.png){#fig-joint1_erc fig-align="center" width="80%"}

![Class-specific fitness mean  predicted trajectory](../../outputs/joint2_erc.png){#fig-joint2_erc fig-align="center" width="80%"}

![Class-specific survival functions](../../outputs/joint3_erc.png){#fig-joint3_erc fig-align="center" width="80%"}
...
```
![Class-specific weighted marginal and subject-specific mean predicted trajectories](../../outputs/joint1_f.png){#fig-joint1_f fig-align="center" width="80%"}

![Class-specific fitness mean  predicted trajectory](../../outputs/joint2_f.png){#fig-joint2_f fig-align="center" width="80%"}
...
```
![Class-specific weighted marginal and subject-specific mean predicted trajectories](../../outputs/joint1_ob.png){#fig-joint1_ob fig-align="center" width="80%"}

![Class-specific fitness mean  predicted trajectory](../../outputs/joint2_ob.png){#fig-joint2_ob fig-align="center" width="80%"}
...
eventlog <- dbGetQuery(con,
                      "SELECT patient_id,event,date,cycle,actins\
                       FROM process_ind WHERE actins NOT IN (\
...
df_3 = id2process_fitness(con,
                          ...
                          event_col = 'event',
                          date_col='date')
df = con.sql("SELECT d.*, p.type AS type,p.date AS date_fin \
              FROM (SELECT *,1 AS year \
              FROM df_1 UNION ALL SELECT *,2 AS year \
              FROM df_2 UNION ALL SELECT *,3 AS year FROM df_3) d \
