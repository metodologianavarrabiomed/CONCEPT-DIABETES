...
con <- dbConnect(duckdb(), dbdir = db_path)
df <- dbGetQuery(con,"SELECT a.*, b.param_value FROM (SELECT pp.*, pc.initial_status AS cpc \
            ...
            patient_condition_hosp pc ON pp.ID=pc.patient_id) a LEFT JOIN \
            hba1c_predx_last_view b ON a.ID = b.patient_id")

dbDisconnect(con, shutdown = TRUE)
df$sex <- as.factor(ifelse(df$sex == 0, 'Male', 'Female'))
df$copayment <- as.factor(df$copayment)
...
fitness_pi_lm <- lm(fitness ~ sex  +time+age_+copayment+cpc, data=df)
rm(df)
...
summary(lm(fitness ~ sex +t_0+age_+copayment, data=df))
