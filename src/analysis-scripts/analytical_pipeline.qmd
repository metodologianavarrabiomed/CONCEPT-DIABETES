...
ggplot(data = df_ %>% filter(t_0<1100), aes(x = t_0, y = fitness, group = ID2))+ 
  geom_line(colour='grey') + 
  #stat_smooth(aes(group = 1),method = "lm",se=TRUE) +
  stat_summary(aes(group = 1), geom = "point", fun = mean, shape = 16, size = 2) + 
...
survfit(Surv(t_min, t_max, status2) ~ class, data=df_ %>%
          select(ID2,t_min,t_max,status2,class) %>% unique()) %>%
  autoplot(xlab = "Day", ylab = "Survival probability")
ggsave(filename=gsub("src/analysis-scripts/","",
...
#### Process indicators' model
```{r}
#| label: tbl-modelsummary2
#| tbl-cap: "Process indicators models' summary"
#| tbl-colwidths: [60,40]
...
#| tbl-cap: "Process indicators joint latent classes' summary"
#| tbl-colwidths: [60,40]
df_ %>% select(ID,ID2,age,sex,copayment,charlson,t_max,
