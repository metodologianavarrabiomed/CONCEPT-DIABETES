...
def separate_patients_by_condition(con):
    ...
    con.sql(f"CREATE OR REPLACE VIEW patient_condition_translation AS \
          ...
          ELSE 'error' END AS final_status, \
...
def id2treat_fitness_by_interval(con,
  ...
  '''
    ...
    for id in tqdm(id_list):
        ...
        date_max = min(cond_end[id],hosp_d)
...
dbExecute(con,
        ...
        SELECT p.*, c.score_ch AS charlson \
...
png(filename=gsub("src/analysis-scripts/","",
                  here("outputs",sprintf("healthcare_dottedchart_%s.png",
                                         cond))),
    width = 600, height = 750, units = "px")
plot(evLog_healthcare %>% 
...
![Visits' dotted chart](../../outputs/healthcare_dottedchart_else.png){#fig-healthcare_dottedchart_else fig-align="center" width="80%"}
...
model <- coxph(Surv(t_0, t_1, status) ~ age + sex + fitness + charlson,
               data=df)
survfit(Surv(t_min, t_max, status2) ~ age_, data=df %>%
  select(ID2,t_min,t_max,status2,age_) %>% unique()) %>%
  autoplot()
...
mj1 <- Jointlcmm(fixed= fitness ~ t_0 ,random =~ t_0, subject="ID2",
                 ...
                 B=c(1,1,0,0,0,0,0,1,0,1,1))
### 2 latent class
tryCatch(
{mj2 <- gridsearch(rep = 20, maxiter = 15, minit = mj1, 
...
png(filename=gsub("src/analysis-scripts/","",
                  here("outputs",sprintf("joint1_%s.png",
                                         cond))),
    width = 600, height = 750, units = "px")
plot(mj2, which = "fit", var.time = "t_0", marg = F, break.times = 10, bty = "l", legend.loc = 'bottomright',
     ylab = "fitness", xlab = "Time in days",shades=TRUE,ylim = c(0.1,1.0),xlim=c(0,1000))
dev.off()
 
d1 <- data.frame(t_0=seq(1,1000,length.out=100))
data.new <-data.frame(d1,age=65,sex='Female',charlson=1)
mj2.pred <- predictY(mj2, data.new , var.time = "t_0")

png(filename=gsub("src/analysis-scripts/","",
                  here("outputs",sprintf("joint2_%s.png",
                                         cond))),
    width = 600, height = 750, units = "px")
plot(mj2.pred, bty = "l", ylim = c(0, 1), legend.loc = 'bottomleft',
...
png(filename=gsub("src/analysis-scripts/","",
                  here("outputs",sprintf("joint3_%s.png",
                                         cond))),
    width = 600, height = 750, units = "px")
plot(mj2, which = "survival", lwd = 2, legend.loc = 'bottomleft', bty = "l",
     xlab = "days", ylab = "event-free probability",xlim=c(0,1000))
dev.off()},
...
df$status3 <- ifelse(df$t_max<=720 & df$status2==1, 1, 0)
df$t_max <- pmin(df$t_max-360,360)

cox.model <- coxph(Surv(t_max, status3) ~ age + sex + fitness + charlson,
               data=df)
...
![Visits' dotted chart](../../outputs/healthcare_dottedchart_cvd.png){#fig-healthcare_dottedchart_cvd fig-align="center" width="80%"}
...
![Visits' dotted chart](../../outputs/healthcare_dottedchart_hf.png){#fig-healthcare_dottedchart_hf fig-align="center" width="80%"}
...
![Visits' dotted chart](../../outputs/healthcare_dottedchart_ckd.png){#fig-healthcare_dottedchart_ckd fig-align="center" width="80%"}
...
![Visits' dotted chart](../../outputs/healthcare_dottedchart_f.png){#fig-healthcare_dottedchart_f fig-align="center" width="80%"}
...
![Visits' dotted chart](../../outputs/healthcare_dottedchart_ob.png){#fig-healthcare_dottedchart_ob fig-align="center" width="80%"}
...
con.sql("CREATE OR REPLACE TABLE window2process_fitness AS \
             ...
             FROM (SELECT * FROM (SELECT df_yyyy.*, \
                  cmbd_incidents_postdx_first_view.admission_date AS admission_date \
                  FROM df_yyyy LEFT JOIN cmbd_incidents_postdx_first_view ON \
                    df_yyyy.ID = cmbd_incidents_postdx_first_view.patient_id) \
...
png(filename=gsub("src/analysis-scripts/","",
    ...
    width = 600, height = 750, units = "px")
plot(mj2, which = "fit", var.time = "t_0", marg = F, break.times = 5, bty = "l",
     legend.loc = 'bottomright',ylab = "fitness",
     xlab = "Time in days",shades=TRUE,ylim = c(0.1,1.0),xlim=c(0,1000),xlim=c(0,1000))
dev.off()
...
cox.model <- coxph(Surv(time, status) ~ age + sex + fitness + charlson,
               data=df)
